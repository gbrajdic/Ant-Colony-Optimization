% ULAZNI PARAMETRI (dodatno objašnjenje je u prezentaciji)
...
phmone=1/(nn_len * Ncity)*ones(Ncity,Ncity);% incicijalizacija feromona izmeðu gradova
...
for it=1:maxit
    ...
    for ia=1:Nants
          ...
          st=tour(ia,iq-1); nxt=tour(ia,iq:Ncity);
           ...
           [~,ind_nxt]=max(value);
           newcity=iq-1+ind_nxt; % sljedeæi grad koji trebamo posjetiti
