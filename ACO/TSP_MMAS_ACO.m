...
function [dbest,best_it]=TSP_MMAS_ACO(cities,Nants,a,b,rr,e,Q,maxit)
...
phmone=(1/(rr*nn_len)) *ones(Ncity,Ncity);% incicijalizacija feromona izmeðu gradova
...
for it=1:maxit
   ...
   % phtemp=zeros(Ncity,Ncity);
    ...
    phmone=(1-rr)*phmone+e*ph1;
    tau_max=1/(rr*dbest);
    tau_min= tau_max*(1-nthroot(0.05,Ncity))/((Ncity/2-1)*nthroot(0.05,Ncity));
   
        %provjera da li je feromon unutar segmenta [tau_min, tau_max] te da
        %li je došlo do stagnacije
for ic=1:Ncity-1
    for id=(ic+1):Ncity
        if((phmone(ic,id)>tau_max)||(phmone(ic,id)<tau_min) || (ii==50))
        phmone(ic,id)=tau_max;
        end
    end % id
end %ic
